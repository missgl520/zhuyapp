...
DEFAULT_OUTPUT_DIR = Path("/tmp/moss_tts_output")
...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        waveform, sample_rate = sf.read(str(Path(reference_audio_path).expanduser().resolve()), dtype='float32')
        if waveform.ndim == 2:
            waveform = waveform.mean(axis=1)  # stereo -> mono
        waveform = waveform.astype(np.float32)
        waveform = waveform[np.newaxis, :]  # (channels=1, samples)
        ...
        if sample_rate != target_sample_rate:
            ...
            waveform = scipy.signal.resample(waveform[0], num_samples)[np.newaxis, :]
